...
interface AdminContractAddresses {
  premiumRegistry: string | null;
  payment: string | null;
...
const ADMIN_CONTRACT_TO_KEY: Record<string, keyof AdminContractAddresses> = {
  PremiumRegistry: 'premiumRegistry',
  Payment: 'payment',
  Banner: 'configBanner',
  TokenWhiteList: 'tokenWhiteList',
...
function buildAdminContractAddresses(networkContracts: Record<string, ContractEntry>): AdminContractAddresses {
  const out: AdminContractAddresses = { premiumRegistry: null, payment: null, configBanner: null, tokenWhiteList: null };
  for (const [contractName, key] of Object.entries(ADMIN_CONTRACT_TO_KEY)) {
    const entry = networkContracts[contractName];
    if (entry?.address) {
      const addr = checksum(entry.address);
      if (addr) out[key] = addr;
...
const ADMIN_ABI_MAPPINGS: Array<{
  artifact: string;
  outPath: string;
  exportName: string;
  style: AbiExportStyle;
}> = [
    { artifact: 'Banner_Implementation.json', outPath: 'admin/blockchain/abis/configBanner.ts', exportName: 'configBannerABI', style: 'default_export' },
    { artifact: 'Payment.json', outPath: 'admin/blockchain/abis/payment.ts', exportName: 'PaymentABI', style: 'default_export' },
    { artifact: 'PremiumRegistry_Implementation.json', outPath: 'admin/blockchain/abis/premiumRegistry.ts', exportName: 'PremiumRegistryABI', style: 'default_export' },
  ];
...
function writeAdminAddresses(): void {
  ...
  const outPath = path.join(OUTPUT_ROOT, 'admin', 'configs', 'contract-addresses.generated.ts');
  ensureDir(path.dirname(outPath));
  fs.writeFileSync(outPath, lines.join('\n'), 'utf-8');
  console.log('Wrote', path.relative(CONTRACTS_ROOT, outPath));
}

function writeAdminAbis(): void {
  const data = fs.readFileSync(CONTRACT_ADDRESSES_PATH, 'utf-8');
  const parsed = JSON.parse(data) as ContractAddressesJson;
  const networks = Object.keys(parsed).filter((n) => NETWORK_CHAIN_IDS[n] !== undefined);
  const deploymentDirs = networks.filter((n) => fs.existsSync(path.join(DEPLOYMENTS_DIR, n)));
  const allDirs =
    fs.existsSync(DEPLOYMENTS_DIR) ?
      ...
      [];
  const network = deploymentDirs[0] ?? allDirs[0] ?? networks[0] ?? 'sepolia';
  for (const m of ADMIN_ABI_MAPPINGS) {
    writeAbiFile(network, m.artifact, m.outPath, m.exportName, m.style);
  }
}
...
function printCopyInstructions(): void {
  ...
  console.log('');
  console.log('UI: Copy output/sync-ui/{configs,constants}/ into the UI repo src/ so that:');
  console.log('     output/sync-ui/configs/  →  10102-ui/src/configs/');
  console.log('     output/sync-ui/constants/  →  10102-ui/src/constants/');
  console.log('');
  console.log('Admin: Copy output/sync-ui/admin/ into the admin repo src/ so that:');
  console.log('     output/sync-ui/admin/configs/  →  10102-admin/src/configs/');
  console.log('     output/sync-ui/admin/blockchain/abis/  →  10102-admin/src/blockchain/abis/');
  console.log('');

...
const deploy: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  console.log("LegacyDeployer deployed to:", data.address);
  await saveContract(network.name, "DefaultProxyAdmin", data.args![1]);
  await saveContract(network.name, "LegacyDeployer", data.address, data.implementation!);

  // Verify implementation only (proxy is standard and may use a different compiler)
  try {

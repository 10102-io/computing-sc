...
const deploy: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const data = await deploy("PremiumAutomationManager", {
    from: deployer,
    args: [],
    log: true,
  ...
  console.log("PremiumAutomationManager deployed to:", data.address);
  await saveContract(network.name, "DefaultProxyAdmin", data.args![1]);
  await saveContract(network.name, "PremiumAutomationManager", data.address, data.implementation!);
...
deploy.tags = ["PremiumAutomationManager"];
